...
const handlers = {
    ...
    "resource-file": {
        install: function (obj, plugin, project, options) {
            ...
            if (fs.existsSync(destFile)) {
                throw new CordovaError(
                    `File already exists at destination "${destFile}" for resource file specified by plugin ${plugin.id} in iOS platform`
                );
            }
    ...
    framework: {
        ...
        install: function (obj, plugin, project, options) {
            ...
            if (!fs.existsSync(srcFile))
                throw new CordovaError(
                    `Cannot find framework "${srcFile}" for plugin ${plugin.id} in iOS platform`
                );
            ...
            const project_relative = fixPathSep(
                path.relative(project.projectDir, targetDir)
            );
            // CB-11233 create Embed Frameworks Build Phase if does not exist
            const existsEmbedFrameworks = project.xcode.buildPhaseObject(
        ...
        uninstall: function (obj, plugin, project, options) {
            ...
            const targetDir = fixPathSep(
                path.resolve(project.plugins_dir, plugin.id, path.basename(src))
            );
            const pbxFile = project.xcode.removeFramework(targetDir, {
    ...
    "lib-file": {
        install: function (obj, plugin, project, options) {
            events.emit(
                "verbose",
                "<lib-file> install is not supported for iOS plugins"
            );
        },
        uninstall: function (obj, plugin, project, options) {
            events.emit(
                "verbose",
                "<lib-file> uninstall is not supported for iOS plugins"
            );
        }
    },
    asset: {
    ...
    "js-module": {
        install: function (obj, plugin, project, options) {
            ...
            if (options && options.usePlatformWww) {
                const platformWwwDestination = path.resolve(
                    project.platformWww,
                    "plugins",
                    plugin.id,
                    obj.src
...
function installHelper(
    ...
    if (link) {
        ...
        project_ref = `Plugins/${fixPathSep(
            path.relative(fs.realpathSync(project.plugins_dir), trueSrc)
        )}`;
    } else {
        if (options && options.force) {
        ...
        project_ref = `Plugins/${fixPathSep(
            path.relative(project.plugins_dir, destFile)
        )}`;
    }
    ...
    if (type === "header-file") {
        ...
        const project_relative = path.join(
            path.basename(project.xcode_path),
            project_ref
        );
        project.xcode.addFramework(project_relative, opt);
