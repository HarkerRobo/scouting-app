...
function getDefaultSimulatorTarget() {
    ...
    return require("./listEmulatorBuildTargets")
        .run()
        .then((emulators) => {
            let targetEmulator;
...
module.exports.run = (buildOpts) => {
    ...
    if (buildOpts.debug && buildOpts.release) {
        return Promise.reject(
            new CordovaError(
                'Cannot specify "debug" and "release" options together.'
            )
        );
    }
    ...
    if (buildOpts.device && buildOpts.emulator) {
        return Promise.reject(
            new CordovaError(
                'Cannot specify "device" and "emulator" options together.'
            )
        );
    }
    ...
    if (buildOpts.buildConfig) {
        if (!fs.existsSync(buildOpts.buildConfig)) {
            return Promise.reject(
                new CordovaError(
                    `Build config file does not exist: ${buildOpts.buildConfig}`
                )
            );
        }
        events.emit(
            "log",
            `Reading build config file: ${path.resolve(buildOpts.buildConfig)}`
        );
    ...
    return require("./listDevices")
        ...
        .then(() => {
            ...
            if (!buildOpts.device) {
                ...
                const promise =
                    require("./listEmulatorBuildTargets").targetForSimIdentifier(
                        newTarget
                    );
                return promise.then((theTarget) => {
                    if (!theTarget) {
                        return getDefaultSimulatorTarget().then(
                            (defaultTarget) => {
        ...
        .then((name) => {
            ...
            function writeCodeSignStyle(value) {
                ...
                events.emit(
                    "verbose",
                    `Set CODE_SIGN_STYLE Build Property to ${value}.`
                );
                project.xcode.updateBuildProperty("CODE_SIGN_STYLE", value);
                events.emit(
                    "verbose",
                    `Set ProvisioningStyle Target Attribute to ${value}.`
                );
        ...
        .then(() => {
            ...
            events.emit(
                "log",
                `\tPlatform: ${buildOpts.device ? "device" : "emulator"}`
            );
            ...
            const xcodebuildArgs = getXcodeBuildArgs(
                projectName,
                projectPath,
                configuration,
                buildOpts.device,
            ...
            return spawn("xcodebuild", xcodebuildArgs, {
                cwd: projectPath,
                printCommand: true,
                stdio: "inherit"
            });
        ...
        .then(() => {
            ...
            function packageArchive() {
                const xcodearchiveArgs = getXcodeArchiveArgs(
                    projectName,
                    projectPath,
                    buildOutputDir,
                    exportOptionsPath,
                ...
                return spawn("xcodebuild", xcodearchiveArgs, {
                    cwd: projectPath,
                    printCommand: true,
                    stdio: "inherit"
                });
            ...
            return fs
                .writeFile(exportOptionsPath, exportOptionsPlist, "utf-8")
...
function findXCodeProjectIn(projectPath) {
    ...
    const xcodeProjFiles = fs
        ...
        .filter((name) => path.extname(name) === ".xcodeproj");

    if (xcodeProjFiles.length === 0) {
        return Promise.reject(
            new CordovaError(`No Xcode project found in ${projectPath}`)
        );
    }
...
function getXcodeBuildArgs(
    ...
    if (isDevice) {
        ...
        options = [
            ...
            "-destination",
            customArgs.destination ||
                `platform=iOS Simulator,name=${emulatorTarget}`
        ];
        buildActions = ["build"];
        settings = [
            customArgs.configuration_build_dir ||
...
function parseBuildFlag(buildFlag, args) {
    ...
    for (const key in buildFlagMatchers) {
        ...
        if (found) {
            ...
            events.emit(
                "warn",
                util.format("Overriding xcodebuildArg: %s", buildFlag)
            );
        }
    }
    ...
    if (!matched) {
        ...
        if (
            ...
            events.emit(
                "warn",
                util.format("Adding xcodebuildArg: %s", buildFlag.split(" "))
            );
        } else {
            args.otherFlags.push(buildFlag);
            events.emit(
                "warn",
                util.format("Adding xcodebuildArg: %s", buildFlag)
            );
        }
    }
}
...
module.exports.help = function help() {
    console.log("");
    console.log(
        'Usage: build [--debug | --release] [--archs="<list of architectures...>"]'
    );
    console.log(
        '             [--device | --simulator] [--codeSignIdentity="<identity>"]'
    );
    console.log(
        '             [--codeSignResourceRules="<resourcerules path>"]'
    );
    ...
    console.log(
        "    --debug                 : Builds project in debug mode. (Default)"
    );
    console.log(
        "    --release               : Builds project in release mode."
    );
    console.log(
        "    -r                      : Shortcut :: builds project in release mode."
    );
    /* eslint-enable no-useless-escape */
    ...
    console.log(
        "                            : Specifies, what type of project to build"
    );
    console.log(
        "    --codeSignIdentity      : Type of signing identity used for code signing."
    );
    console.log("    --codeSignResourceRules : Path to ResourceRules.plist.");
    console.log(
        "    --developmentTeam       : New for Xcode 8. The development team (Team ID)"
    );
    console.log("                              to use for code signing.");
    console.log("    --provisioningProfile   : UUID of the profile.");
    console.log(
        "    --device --noSign       : Builds project without application signing."
    );
    console.log("");
