...
function BridgingHeader(bridgingHeaderPath) {
    ...
    this.bridgingHeaders = this.__parseForBridgingHeader(
        fs.readFileSync(this.path, "utf8")
    );
}

BridgingHeader.prototype.addHeader = function (plugin_id, header_path) {
    this.bridgingHeaders.push({
        type: "code",
        code: `#import "${header_path}"\n`
    });
};
...
BridgingHeader.prototype.__stringifyForBridgingHeader = function (
    bridgingHeaders
) {
