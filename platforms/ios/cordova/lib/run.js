...
module.exports.run = (runOptions) => {
    ...
    if (runOptions.device && runOptions.emulator) {
        return Promise.reject(
            new CordovaError(
                'Only one of "device"/"emulator" options should be specified'
            )
        );
    }
    ...
    if (runOptions.list) {
        ...
        return module.exports
            .listDevices()
            .then(() => module.exports.listEmulators());
    }
            ...
            if (useDevice) {
                return module.exports
                    ...
                    .then(() => {
                        ...
                        const appFileInflated = path.join(
                            buildOutputDir,
                            "Payload",
                            `${projectName}.app`
                        );
...
function deployToDevice(appPath, target, extraArgs) {
    ...
    if (target) {
        return spawn(
            "ios-deploy",
            ["--justlaunch", "-d", "-b", appPath, "-i", target].concat(
                extraArgs
            ),
        ...
        return spawn(
            "ios-deploy",
            ["--justlaunch", "--no-wifi", "-d", "-b", appPath].concat(
                extraArgs
            ),
            { printCommand: true, stdio: "inherit" }
        );
...
function deployToSim(appPath, target) {
    ...
    if (!target) {
        ...
        return require("./listEmulatorImages")
            .run()
            .then((emulators) => {
...
module.exports.help = () => {
    ...
    console.log(
        "    --device      : Deploys and runs the project on the connected device."
    );
    console.log(
        "    --emulator    : Deploys and runs the project on an emulator."
    );
    console.log(
        "    --target=<id> : Deploys and runs the project on the specified target."
    );
    console.log(
    ...
    console.log(
        "    --nobuild     : Uses pre-built package, or errors if project is not built."
    );
    // TODO: add support for building different archs
    ...
    console.log(
        '    run --emulator --target="iPhone-6-Plus"'
