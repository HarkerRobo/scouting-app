...
const IOS_DEPLOY_NOT_FOUND_MESSAGE = `Please download, build and install version ${IOS_DEPLOY_MIN_VERSION} or greater from https://github.com/ios-control/ios-deploy into your path, or do 'npm install -g ios-deploy'`;
...
module.exports.run = module.exports.check_xcodebuild = () => {
    return checkTool(
        "xcodebuild",
        XCODEBUILD_MIN_VERSION,
        XCODEBUILD_NOT_FOUND_MESSAGE
    );
};
...
module.exports.check_os = () => {
    ...
    return os_platform_is_supported()
        ...
        : Promise.reject(
              new CordovaError("Cordova tooling for iOS requires Apple macOS")
          );
};
...
module.exports.check_cocoapods = (toolChecker) => {
    if (os_platform_is_supported()) {
        return checkTool(
            "pod",
            COCOAPODS_MIN_VERSION,
            COCOAPODS_NOT_FOUND_MESSAGE,
            "CocoaPods"
        );
    }
...
function checkTool(tool, minVersion, message, toolFriendlyName) {
    ...
    if (!tool_command) {
        return Promise.reject(
            new CordovaError(
                `${toolFriendlyName} was not found. ${message || ""}`
            )
        );
    }
    ...
    return versions.get_tool_version(tool).then((version) => {
        ...
        return versions.compareVersions(version, minVersion) >= 0
            ...
            : Promise.reject(
                  new CordovaError(
                      `Cordova needs ${toolFriendlyName} version ${minVersion} or greater, you have version ${version}. ${
                          message || ""
                      }`
                  )
              );
