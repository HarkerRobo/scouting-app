...
module.exports.prepare = function (cordovaProject, options) {
    ...
    const munger = new PlatformMunger(
        "ios",
        this.locations.root,
        platformJson,
...
function updateWww(cordovaProject, destinations) {
    ...
    events.emit(
        "verbose",
        `Merging and updating files from [${sourceDirs.join(
            ", "
        )}] to ${targetDir}`
    );
...
function updateProject(platformConfig, locations) {
    ...
    const CFBundleVersion =
        platformConfig.getAttribute("ios-CFBundleVersion") ||
    ...
    if (platformConfig.getAttribute("defaultlocale")) {
        infoPlist.CFBundleDevelopmentRegion =
            platformConfig.getAttribute("defaultlocale");
    }
    ...
    events.emit(
        "verbose",
        `Wrote out iOS Bundle Version "${version}" to ${plistFile}`
    );
...
function handleBuildSettings(platformConfig, locations, infoPlist) {
    const pkg =
        platformConfig.getAttribute("ios-CFBundleIdentifier") ||
        platformConfig.packageName();
    const targetDevice = parseTargetDevicePreference(
        platformConfig.getPreference("target-device", "ios")
    );
    const deploymentTarget = platformConfig.getPreference(
        "deployment-target",
        "ios"
    );
    ...
    try {
        ...
        return Promise.reject(
            new CordovaError(`Could not parse ${locations.pbxproj}: ${err}`)
        );
    }

    const origPkg = project.xcode.getBuildProperty(
    ...
    if (origPkg !== pkg) {
        ...
        project.xcode.updateBuildProperty(
            "PRODUCT_BUNDLE_IDENTIFIER",
            pkg,
            null,
            platformConfig.name()
    ...
    if (targetDevice) {
        events.emit(
            "verbose",
            `Set TARGETED_DEVICE_FAMILY to ${targetDevice}.`
        );
        project.xcode.updateBuildProperty(
    ...
    if (deploymentTarget) {
        events.emit(
            "verbose",
            `Set IPHONEOS_DEPLOYMENT_TARGET to "${deploymentTarget}".`
        );
        project.xcode.updateBuildProperty(
...
function mapIconResources(icons, iconsDir) {
    ...
    platformIcons.forEach((item) => {
        const icon =
            icons.getBySize(item.width, item.height) || icons.getDefault();
        if (icon) {
...
function getIconsDir(projectRoot, platformProjDir) {
    let iconsDir;
    const xcassetsExists = folderExists(
        path.join(projectRoot, platformProjDir, "Images.xcassets/")
    );
...
function updateIcons(cordovaProject, locations) {
    ...
    const platformProjDir = path.relative(
        cordovaProject.root,
        locations.xcodeCordovaProj
    );
...
function updateBackgroundColor(cordovaProject, locations) {
    const pref =
        cordovaProject.projectConfig.getPreference("BackgroundColor", "ios") ||
        "";
    ...
    const backgroundColorDir = getBackgroundColorDir(
        cordovaProject.root,
        platformProjDir
    );
    ...
    if (backgroundColorDir) {
        ...
        fs.writeFileSync(
            path.join(cordovaProject.root, backgroundColorDir, "Contents.json"),
...
function cleanBackgroundColor(projectRoot, projectConfig, locations) {
    ...
    if (backgroundColorDir) {
        ...
        events.emit(
            "verbose",
            "Cleaning Background Color color set Contents.json"
        );
        fs.writeFileSync(
            path.join(projectRoot, backgroundColorDir, "Contents.json"),
            JSON.stringify(contentsJSON, null, 2)
        );
    }
}
...
function updateFileResources(cordovaProject, locations) {
    ...
    files.forEach((res) => {
        ...
        if (!fs.existsSync(targetPath)) {
            ...
            events.emit(
                "warn",
                `Overwriting existing resource file at ${targetPath}`
            );
        }
...
function alertDeprecatedPreference(configParser) {
    ...
    Object.keys(deprecatedToNewPreferences).forEach((oldKey) => {
        if (configParser.getPreference(oldKey)) {
            ...
            if (newPreferenceKey) {
                log.push(
                    `It is recommended to replace this preference with "${newPreferenceKey}."`
                );
            } else {
            ...
            if (!isDeprecated) {
                log.push(
                    "Please note that this preference will be removed in the near future."
                );
            }
 ...
 * @return {Object}
 */
function mapLaunchStoryboardResources(
    splashScreens,
    launchStoryboardImagesDir
) {
    const platformLaunchStoryboardImages = mapLaunchStoryboardContents(
        splashScreens,
        launchStoryboardImagesDir
    );
    const pathMap = {};
...
function getLaunchStoryboardContentsJSON(
    ...
    contentsJSON.images = platformLaunchStoryboardImages.map((item) => {
        ...
        if (item.appearence) {
            newItem.appearances = [
                { appearance: "luminosity", value: item.appearence }
            ];
        }
...
function updateLaunchStoryboardImages(cordovaProject, locations) {
    ...
    const launchStoryboardImagesDir = getLaunchStoryboardImagesDir(
        cordovaProject.root,
        platformProjDir
    );

    if (launchStoryboardImagesDir) {
        const resourceMap = mapLaunchStoryboardResources(
            splashScreens,
            launchStoryboardImagesDir
        );
        const contentsJSON = getLaunchStoryboardContentsJSON(
            splashScreens,
            launchStoryboardImagesDir
        );
        ...
        events.emit(
            "verbose",
            `Updating launch storyboard images at ${launchStoryboardImagesDir}`
        );
        FileUpdater.updatePaths(
            resourceMap,
            { rootDir: cordovaProject.root },
...
function cleanLaunchStoryboardImages(projectRoot, projectConfig, locations) {
    ...
    if (launchStoryboardImagesDir) {
        ...
        events.emit(
            "verbose",
            `Cleaning storyboard image set at ${launchStoryboardImagesDir}`
        );
...
function getOrientationValue(platformConfig) {
    ...
    events.emit(
        "warn",
        `Unrecognized value for Orientation preference: ${orientation}. Defaulting to value: ${ORIENTATION_DEFAULT}.`
    );
            ...
            .reduce((previousReturn, currentElement) => {
                const options = {
                    ...
                    requires_forward_secrecy:
                        currentElement.requires_forward_secrecy,
                    ...
                    allows_local_networking:
                        currentElement.allows_local_networking
                };
                const obj = parseWhitelistUrlForATS(
                    currentElement.href,
                    options
                );
...
function parseWhitelistUrlForATS(url, options) {
    ...
    if (
        ...
        options.minimum_tls_version !== "TLSv1.2"
    ) {
        // default is TLSv1.2
...
function parseTargetDevicePreference(value) {
    ...
    events.emit(
        "warn",
        `Unrecognized value for target-device preference: ${value}.`
    );
    return null;
}
