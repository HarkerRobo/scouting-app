...
<div class="login-window">
	...
	<input type="password" placeholder="Key" name="Key" value="${this.escape(
        await getKey()
    )}" /><br>
	<button>Login</button><br>
</div>`;
                element.querySelector(".login-window > button").onclick =
                    async () => {
                        let username = element.querySelector(
                            ".login-window > input[name='Username']"
                        ).value;
                        let key = element.querySelector(
                            ".login-window > input[name='Key']"
                        ).value;
...
<option value="${this.escape(teamNumber)}"${
    ...
    this.showScannerPage = (eventCode) => {
        return new Promise(async (resolve, reject) => {
            if ((await this.getUser()) == "") {
                ...
                element.querySelector("button.scan-again").onclick =
                    async () => {
                        try {
                            await reader.stop();
                        } catch (err) {}
        ...
        return new Promise(async (resolve, reject) => {
            if ((await this.getUser()) == "") {
                ...
                element.querySelector("button.download-csv").onclick =
                    async () => {
                        let eventCode = element.querySelector(
                            ".download-window > input.event-code"
                        ).value;
    ...
    this.getQuote = () => {
        return new Promise(async (resolve, reject) => {
            ...
            if (randomQuoteID < importantQuotes.length * 3) {
                ...
                boltmanQuote =
                    quotes[randomQuoteID - importantQuotes.length * 3];
            }
    ...
    this.login = (username, key) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (
                config.upload.passwordHash == "" ||
                config.upload.passwordHash == hashed
            ) {
            ...
            try {
                let matches = await (
                    await fetch(
                        `${
                            config.upload.endpoint
                        }/matches?key=${encodeURIComponent(
                            await getKey()
                ...
                if (matches.success) {
                    localStorage.setItem(
                        `matches::${eventCode}`,
                        JSON.parse(JSON.stringify(matches.contents || "[]"))
                    );
                    resolve(true);
                } else {
                    resolve(false);
            ...
            if (component.type == "layout") {
                ...
                pendingFunctions.push(async () => {
                    element.querySelector(
                        ...
                        ).innerHTML = this.timerFormat(data[component.data]);
                    };
                    element.querySelector(
                        `[data-id="${this.escape(
                            id
                ...
                pendingFunctions.push(async () => {
                    try {
                        ...
                        let upload = await (
                            await fetch(`${config.upload.endpoint}/upload`, {
                                method: "POST",
                                headers: {
                                    "Content-Type":
                                        "application/x-www-form-urlencoded;charset=UTF-8"
