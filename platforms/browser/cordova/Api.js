...
Api.createPlatform = function (dest, config, options, events) {
    ...
    try {
        ...
        result = creator
            .createProject(dest, id, name, options)
            .then(function () {
        ...
        events.emit(
            "error",
            "createPlatform is not callable from the browser project API."
        );
        throw e;
    }
    return result;
...
Api.prototype.prepare = function (cordovaProject, options) {
    ...
    if (fs.existsSync(defaultConfigPath)) {
        this.events.emit(
            "verbose",
            'Generating config.xml from defaults for platform "' +
                this.platform +
                '"'
        );
        ...
        this.events.emit(
            "verbose",
            'Generating defaults.xml from own config.xml for platform "' +
                this.platform +
                '"'
        );
    ...
    var srcManifestPath = path.join(
        cordovaProject.locations.www,
        "manifest.json"
    );
    if (fs.existsSync(srcManifestPath)) {
        ...
        this.events.emit(
            "verbose",
            "copying " + srcManifestPath + " => " + manifestPath
        );
        ...
        if (this.config) {
            ...
            // get start_url
            var contentNode = this.config.doc.find("content") || {
                attrib: { src: "index.html" }
            }; // sensible default
            manifestJson.start_url = contentNode.attrib.src;

            // now we get some values from start_url page ...
            var startUrlPath = path.join(
                cordovaProject.locations.www,
            ...
            if (fs.existsSync(startUrlPath)) {
                ...
                var themeColorRegex =
                    /<meta(?=[^>]*name="theme-color")\s[^>]*content="([^>]*)"/i;
                ...
                if (result && result.length >= 2) {
                    ...
                    themeColor = this.config.getGlobalPreference(
...
Api.prototype.addPlugin = function (pluginInfo, installOptions) {
    ...
    installOptions.platformVersion =
        installOptions.platformVersion || this.getPlatformInfo().version;

    var self = this;
    var actions = new ActionStack();
    var projectFile =
        this._handler.parseProjectFile &&
    ...
    return actions.process(this.platform, this.root).then(function () {
        ...
        if (!installOptions.variables.PACKAGE_NAME) {
            installOptions.variables.PACKAGE_NAME = self._handler.package_name(
                self.root
            );
        }
...
Api.prototype._getUninstaller = function (type) {
    ...
    return function (item, plugin_dir, plugin_id, options, project) {
        ...
        if (!installer) {
            console.log(
                "browser plugin uninstall: unrecognized type, skipping : " +
                    type
            );
        } else {
            var wwwDest = options.usePlatformWww
...
Api.prototype._writePluginModules = function (targetDir) {
    ...
    fs.writeFileSync(
        path.join(targetDir, "cordova_plugins.js"),
        final_contents,
        "utf-8"
    );
};
